...
from tsp.model.base import TspDecoder, TspCritic
...
class TestModel(unittest.TestCase):
    def _run_checks_decoder(self, batch_size, dim_model, problem_size):
        coords = get_coords(batch_size, problem_size)
        node_encodings = make_encodings(coords, dim_model, 1)
        pad_mask = generate_padding_mask(coords)

        decoder = TspDecoder(dim_model, num_layers=2)
    ...
    def test_decoder(self):
        ...
        self._run_checks_decoder(batch_size, dim_model, problem_size)

    def test_decoder_EOS_pad_masking(self):
        batch_size = 32  # N
        dim_model = 128  # d
        problem_size = range(4, 7)  # S
