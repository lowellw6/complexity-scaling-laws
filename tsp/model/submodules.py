...
class TspEncoder(nn.Module):
    ...
    def forward(self, problems):
        """
        Assumes 'problems' has shape (N, S, d)
            N = batch dim
            S = seq dim
            d = TSP data spatial dim
        """
...
class PositionalEncoding(nn.Module):
    ...
    this module assumes data is already shaped (S, N, d),
    where the sequence dim comes **before** the batch dim,
    in contrast to the (N, S, d) shape seen elsewhere.
...
class TspCritic(nn.Module):
    ...
    def __init__(self, dim_model=128, num_layers=2, tsp_dimensions=2):
        super().__init__()
        self.dim_model = dim_model
        self.num_layers = num_layers
        self.tsp_dimensions = tsp_dimensions
    ...
    def forward(
        ...
        # decoder step with language-model-style tgt and mem (not flipped like in TspDecoder)
